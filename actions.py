...
class AreaForm(FormAction):
    ...
    def slot_mappings(self) -> Dict[Text, Any]:
        return {"area": self.from_entity(entity = "area",
                                         intent = ["info_area"])}
    ...
    def submit(self,
        ...
        if len(results) == 0:
            dispatcher.utter_message(
                "Disculpa, no pudimos encontrar un proyecto en {}.".format(button_name))
            return []
        ...
        if len(buttons) == 1:
            message = "Te recomiendo este proyecto en el contexto de {}".format(area)
        else:
            message = "Te recomiendo estos {} proyectos en el contexto de {}:".format(len(buttons),
                                                         button_name)
